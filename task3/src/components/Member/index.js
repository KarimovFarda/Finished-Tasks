...
const Member = () => {
    ...
    const onInputChange = useCallback((e) => {
        setNewClient(() => {
            return { [e.target.name]: e.target.value, [e.target.id] : member.length + 1, id : member.length + 1 };
        });
    }, [member])
    const addMemberSubmit = useCallback((e) => {
        e.preventDefault() 
        let isValid = true;
        if(!newClient.member_name){
            isValid= false
        }        
        if(isValid){ addMembers(newClient)(dispatch)
